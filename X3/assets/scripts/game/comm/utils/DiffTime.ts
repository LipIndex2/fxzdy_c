...
export class DiffTime {
    ...
    static createByAtTimeMs(atTimeMs: number,
                            currentTimeMs: number = G.TimeManager.serverNow
    ): DiffTime {
        const diffTimeMs = atTimeMs - currentTimeMs;
    ...
    toMaxPastTimeText(maxUnitCount: number = 2): string {
        ...
        if (this.diffDays > 0) {
            return DateUtils.dateTimeFormat(this.atTimeMs, "yyyy-MM-dd HH:mm")
        }
     ...
     * 任务过期描述文本
     */
    toTaskExpireTimeDescText(): string {
        // if (this.type == DiffTimeType.PAST) {
        //     return "已过期";
        // }
        //
        // if (this.diffDays > 0) {
        //     return `${this.diffDays}天后重置`;
        // }
        // if (this.diffHours > 0) {
        //     return `${this.diffHours}小时后重置`;
        // }
        // return `${this.diffMinutes}分钟后重置`;
        const hour = DateUtils.dateTimeFormat(this.atTimeMs, "h");
        const timeUnitHourPoint = G.I18nManager.translate(TimeI18nKeys.timeUnitHourPoint)
        const reset = G.I18nManager.translate(CommonI18nKeys.reset)
        const everyDay = G.I18nManager.translate(TimeI18nKeys.everyDay)
        return `${everyDay}${hour}${timeUnitHourPoint}${reset}`
    }
