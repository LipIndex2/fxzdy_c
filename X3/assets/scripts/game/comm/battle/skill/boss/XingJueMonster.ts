...
export class XingJueMonsterSkill1 extends FightSkillInfo {
    /***开始触发行为 */
            ...
            if (exData) {
                ...
                if (P4311_p101Parm?.flag) {
                    if (taker.isActive) {
                        ...
                        let passSkillData = behavior.skill.owner.attr.getSkillByBeLongType("218_p101") as PassivitySkillData;
...
export class XingJueMonsterPassivitySkill1 extends FightSkillInfo {
    /***buff执行某行为后 */
    public buffAfterExData(buff: SkillBuff, data: { elementRecursionBoom: boolean }): void {
        if (data?.elementRecursionBoom) {
            let P4311_x101Parm: { buff: string, num: number } = this.skill.owner.attr.getPassiveSkillFlag(PassivitySkillFlag.P4311_x101);
...
export class XingJueMonsterSkill3 extends XingJueMonsterSkill1 {
