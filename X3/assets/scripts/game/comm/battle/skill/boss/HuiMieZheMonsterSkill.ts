...
import { HuiMieZheMonster } from "./HuiMieZheMonster";

export class HuiMieZheMonsterSkill2 extends FightSkillInfo {
    /***开始触发行为 */
...
export class HuiMieZheMonsterSkill3 extends FightSkillInfo {
...
export class HuiMieZheMonsterPassivitySkill1 extends FightSkillInfo {
    private haveBuff: boolean = false
    /***开始触发行为 */
    public beginBehaviorEffect(behavior: SkillBehavior, owner: HuiMieZheMonster): void {
        let param: { hp: number, buff: string, skill2: number, num: number, notDie: number } = behavior.cfg.param;
        if (param?.notDie && !owner.canPassivitySkill1) {
            return
        }
        super.beginBehaviorEffect(behavior, owner)
        if (param?.skill2) {
            for (let k = 0; k < owner.attr.passSkills.length; k++) {
                if (owner.attr.passSkills[k].cfg.group == "214_s201") {
