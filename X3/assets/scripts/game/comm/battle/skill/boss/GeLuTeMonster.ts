...
export class GeLuTeShowMonster extends MonsterShowUnit {
    /***是否变身形态 */
    ...
    protected setStateHandler(actionName: string, loop: boolean, timeScaler?: number): void {
        if (actionName == "die" && (this.unitData as GeLuTeMonster).sleepTime) {
            actionName = "Pskill01"
        }
        if (this.isChange) {
            actionName += "2";
    ...
    public removeBuff(buff: SkillBuff): void {
        ...
        if (buff.skillBuffGroup.cfg.flag == BuffGroupFlagType.GeLuTe) {
            if ((this.unitData as GeLuTeMonster).buffGroupId && this.unitData.isDeath) {
            }
            else {
                this.isChange = false;
            }
    ...
    protected onDie(damageVo?: DamageVo) {
        ...
        if ((this.unitData as GeLuTeMonster).sleepTime) {
...
export class GeLuTeMonster extends MonsterUnit {
    ...
    protected onDie(damageVo?: DamageVo) {
        ...
        if (P1141_p101Parm) {
            let myUnits = this.battleLogic.unitProcessor.getUnitsByTeamId(WorldUnitTeam.Enemy);
    ...
    /***获取当前绑定的对应显示单位，不一定有值 */
    public showUnit(): GeLuTeShowMonster {
        return super.showUnit() as GeLuTeShowMonster;
    }
