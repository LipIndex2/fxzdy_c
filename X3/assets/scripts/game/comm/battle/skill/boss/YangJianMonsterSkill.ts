...
export class YangJianMonsterSkill2 extends FightSkillInfo {
    ...
    public beginBehaviorEffect(behavior: SkillBehavior, owner: ICaster): void {
        ...
        if (param?.delay && this.selectUnits?.length > 0) {
            ...
            for (let i = 0; i < summons.length; i++) {
                ...
                summons[i].setPosXY(this.skill.owner.dirction == DirctionType.Rigth ? (this.selectUnits[0].pos.x - 60) : (this.selectUnits[0].pos.x + 60), this.selectUnits[i].pos.y)
                summons[i].updatePos()
...
export class YangJianMonsterSkill3 extends FightSkillInfo {
