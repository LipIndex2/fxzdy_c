...
export class HeYaShow extends HeroShowUnit {
    /***是否变身形态 */
    ...
    protected getActionEffectData(): table.battle.SkillEffectConfig {
        if (this.isHeYaChange) {
            ...
            if (this.unitData.skillInfo.skillIndex == 2) {
                let P1120_s203Parm = this.unitData.attr.getPassiveSkillFlag(PassivitySkillFlag.P1120_s203)
                if (P1120_s203Parm) {
                    return TableManager.getDataById(table.battle.SkillEffectConfig, "1120_s4")
                }
            }
        }
        ...
        if (behavior.selectUnits) {
            ...
            if (param && param.targetBuff) {
                for (let i = 0; i < behavior.selectUnits.length; i++) {
                    if (behavior.selectUnits.length == 1) {
                        if (behavior.selectUnits[i] == owner) {
                            //和雅1个的话是双倍
                            owner.battleLogic.buffMgr.buffControlByGroup(param.myBuff, owner, behavior.selectUnits[i], behavior)
                        }
                    }
                    else {
                        owner.battleLogic.buffMgr.buffControlByGroup(param.targetBuff, owner, behavior.selectUnits[i], behavior)
                    }
                }
