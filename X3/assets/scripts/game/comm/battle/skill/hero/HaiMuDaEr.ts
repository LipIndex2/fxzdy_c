...
export class HaiMuDaEr extends HeroUnit {
    /***免疫BUFF的触发次数 */
    // public buffNum: number = 0;
}

export class HaiMuDaErSkill3 extends FightSkillInfo {
    ...
    //         behavior.tempAddDamageValue += P5220_x101Parm.amount * (caster as HaiMuDaEr).buffNum;
    //     }
    ...
    public skillCompleteHandler(): void {
        // (this.skill.owner as HaiMuDaEr).buffNum = 0;
    }
}

export class HaiMuDaErPassivitySkill1 extends FightSkillInfo {
    /***buff执行后 */
    public buffAfter(buff: SkillBuff): void {
        ...
        if (P5220_x101Parm?.buff == buff.cfg.group) {
            ...
            // (this.skill.owner as HaiMuDaEr).buffNum++;
        }
    }
}