 ...
 * 上阵技能展示
*/
@bindFguiExtension('ui://comm/FormationSkillIcon')
export class FormationSkillIcon extends fgui.GComponent {
    /**id*/
    ...
    /**是否有数据*/
    protected _isHaveData: boolean = false;

    get view(): ui.comm.formation.FormationSkillIcon {
        return this as any
    }
    ...
    public updateData(baseId: number, star: number, lv:number = -1): void {
        ...
        if (this._baseId != baseId) {
            ...
            if (this._type == FormationSkillType.COLLECTIONS) {
                ...
                if (itemCfg) {
                    ...
                    this.view.bgQuality.icon = qualityCfg?.petHeadQualityBgPath
                    this.view.iconLoader.icon = itemCfg?.iconPath;
                    this._skillName = itemCfg.name;
                    this._skillQuality = itemCfg.quality;
                    this.view.getController('iconState').selectedIndex = 0;
                ...
                if (petCfg) {
                    ...
                    this.view.getController('iconState').selectedIndex = 1;
                    this._isHaveData = true;
                }
            }
        }
        ...
        if (this._type == FormationSkillType.COLLECTIONS) {
            ...
            if (isLimit == false) {
                //非限时收藏品才展示等级
                let realLv = lv >= 0 ? lv : GIns.collectionsModel.context.getCollectionLV(this._baseId);
                this.view.lbLv.visible = true;
                this.view.lbLv.text = '+' + realLv;
            }
        }
    }
    ...
    public updateByVisitVo(vo: FormationSkillVisitVo): void {
        if (vo) {
            if (this._type == FormationSkillType.COLLECTIONS) {
                this.updateData((vo as Vo.formation.CollectiblesVisitVo).collectiblesId, vo.star, vo.level);
            } else if (this._type == FormationSkillType.PET) {
