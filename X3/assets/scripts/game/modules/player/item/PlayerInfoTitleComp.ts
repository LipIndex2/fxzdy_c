...
@bindFguiExtension("ui://playerInfo/PlayerInfoTitleComp")
export class PlayerInfoTitleComp extends FGUI.GComponent implements INotification {


    private _config: table.set.SetShowConfig;

    get view(): ui.playerInfo.item.PlayerInfoTitleComp {
        return this as any;
    }
    ...
    protected onConstruct() {
        ...
        this.view.onClick(() => {
            ...
            if (this._config) {
                context.chooseCache.titleId = this._config.id;
    ...
    reset(config: table.set.SetShowConfig) {
        ...
        const isWear = titleId === settingsId;
        this.view.getController("isWear").selectedIndex = isWear ? 1 : 0;

        // 是否选中
        const isChoose = context.chooseCache.titleId == settingsId;
        this.view.getController("isChoose").selectedIndex = isChoose ? 1 : 0;

        FguiScriptUtils.toMyScriptClass(this.view.redDot, RedDotCom).reset(RedDotKeys.Set_skin_title_item, [config.id])
    }
}