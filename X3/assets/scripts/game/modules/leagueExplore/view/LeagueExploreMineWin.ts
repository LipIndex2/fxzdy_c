...
@bindScript(UILeagueExploreConfig.LeagueExploreMineWin)
export class LeagueExploreMineWin extends UICommWin {
    static pkgName: string = "leagueExplore";
    static viewName: string = "LeagueExploreMineWin";
    ...
    protected _endHangUpTime: number = 0;
    protected _timerKey2: string = null;
    protected _endTime2: number = 0;
    protected _stateTimerDes: string = '';

    private get view(): ui.leagueExplore.view.LeagueExploreMineWin {
    ...
    protected onClickRule(): void {
        RuleController.ins().openRule(EnumRuleKeys.LEAGUE_EXPLORE_MINE, this.view.btnRule);
    }
    ...
    protected onTimer(): void {
        if (this._endHangUpTime == 0) {
            this.view.lbTime.text = '';
            return;
        }
        let nowTime: number = G.TimeManager.serverNow;
        let remainTime: number = this._endHangUpTime - nowTime;
        if (remainTime <= 0) {
            remainTime = 0;
            this.removeTimer();
        }
    ...
    protected updateUI(): void {
        ...
        let assistTarget = GIns.leagueExploreModel.getAssistDefendTargetForMine(this._buildingVo);
        if (assistTarget) {
            this.view.lbOccupyPlayer.color = LeagueExploreUtils.colorForEnemy;
            if (assistTarget['playerName']) {
                //代表是玩家
                this.view.lbOccupyPlayer.text = assistTarget['playerName'];
                this.view.lbOccupyPlayerTitle.text = '支援守卫：';
            } else if (assistTarget['name']) {
                //代表是机器人
                this.view.lbOccupyPlayer.text = assistTarget['name'];
                this.view.lbOccupyPlayerTitle.text = '支援守卫：';
            } else {
                this.view.lbOccupyPlayer.text = ''
            }
        } else {
            this.view.lbOccupyPlayer.color = LeagueExploreUtils.colorForNone;
            this.view.lbOccupyPlayer.text = '无';
            this.view.lbOccupyPlayerTitle.text = '支援守卫：';
        }


        if (this._buildingVo.vo.memberVos.length > 0) {
            let firstMember = this._buildingVo.vo.memberVos[0];
            this._endHangUpTime = firstMember.hangUpStartTime + firstMember.hangUpAdditionMillis + GIns.leagueExploreModel.constCfg.continueMinuets * 60000
            this.view.lbTimeTitle.visible = true;
            this.addTimer();
        } else {
            this.view.lbTimeTitle.visible = false;
            this._endHangUpTime = 0;
            this.onTimer();
        }
        this.updateStateTimer();
    }
    ...
    protected onOpen(args: any, isReopen?: boolean): void {
        ...
        this.view.iconLoader.icon = this._buildingVo.buildingCfg.building_collection;
    }
