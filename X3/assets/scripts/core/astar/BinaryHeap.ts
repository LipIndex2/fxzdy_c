...
export class BinaryHeap {
    ...
    public changeFromBMH(element: any): void {
        ...
        if (parent && parent[this._attributeName] > self[this._attributeName]) {//有父节点且比父节点小 上滤
            this.bHUp(index);
        }
        else {//尝试下滤
            this.bHDown(index);
    ...
    private bHUp(index: number): void {
        ...
        do {
            ...
            var parentId: number = Math.floor(selfIndex / 2);
            var parent: any = this._arr[parentId - 1];
            if (parent && parent[this._attributeName] > self[this._attributeName]) {//父节点大,交换
    ...
    private bHDown(index: number): void {
        ...
        do {
            ...
            if (child2 && child2[this._attributeName] < child1[this._attributeName]) {//有右节点,且更小
                if (child2[this._attributeName] < self[this._attributeName]) {//如果右节点小,置换
            ...
            else if (child1 && child1[this._attributeName] < self[this._attributeName]) {//跟左节点比较,如果左节点小,置换
