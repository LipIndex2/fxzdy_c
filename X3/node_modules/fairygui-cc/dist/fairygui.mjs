import { Asset, AssetManager, assetManager, AudioClip, AudioSourceComponent, BitmapFont, BufferAsset, Color, Component, director, dragonBones, EditBox, Event as Event$1, EventMouse, EventTarget, Font, game, gfx, Graphics, ImageAsset, isValid, Label, Layers, macro, Mask, math, misc, Node, Overflow, path, Rect, resources, RichText, screen, Size, sp, Sprite, SpriteAtlas, SpriteFrame, sys, Texture2D, UIOpacity, UIRenderer, UITransform, Vec2, Vec3, view, View, Tween } from 'cc';
...
Event.LIST_SHOW_END = "fui_list_show_end";
...
class GearBase {
    ...
    setup(buffer) {
        this._controller = this._owner.parent.getControllerAt(buffer.readByte()); //buffer.readShort()
        this.init();
        var i;
        var page;
        var cnt = buffer.readUshort(); //buffer.readShort();
        if ("pages" in this) {
            this.pages = buffer.readSArray(cnt);
        }
        else {
            for (i = 0; i < cnt; i++) {
                page = buffer.readString(); //buffer.readS();
                if (page == null)
                    continue;
...
class GearIcon extends GearBase {
    ...
    addStatus(pageId, buffer) {
        if (!pageId)
            this._default = buffer.readString(); //buffer.readS();
        else
            this._storage[pageId] = buffer.readString(); //buffer.readS();
    }
    apply() {
        this._owner._gearLocked = true;
        var data = this._storage[this._controller.selectedPageId];
...
class GearXY extends GearBase {
    get positionsInPercent() {
        return this._positionsInPercent;
    }
    set positionsInPercent(val) {
        this._positionsInPercent = val;
    }
    init() {
        this._default = {
            x: this._owner.x,
...
class GObject {
    ...
    constructFromResource(async = false, completeHandler = null) {
        if (async) {
            completeHandler();
        }
    }
...
class GGraph extends GObject {
    ...
    set alpha(value) {
        if (this._alpha != value) {
            this._alpha = value;
            this._fillColor.a = 255 * value;
            this.color = this._fillColor;
        }
    }
    /**
     * 修改alpha值，强制修改忽略上次设置判断
     * @param val
     */
    changeAlpha(val) {
        this._alpha = -1;
    ...
    setup_beforeAdd(buffer, beginPos) {
        ...
        if (this._type != 0) {
            ...
            this._fillColor.a = Math.ceil(this.alpha * 255);
            if (this._type === 1) {
                if (buffer.readBool()) {
                    // this._cornerRadius = new Array<number>(4);
                    // for (i = 0; i < 4; i++)
                    //     this._cornerRadius[i] = buffer.readFloat();
                    let corner = buffer.readFloat();
                    this._cornerRadius = [corner, corner, corner, corner];
                }
            }
            else if (this._type === 3) {
                cnt = buffer.readUshort(); //buffer.readShort();
                this._polygonPoints = [];
...
class GImage extends GObject {
    ...
    get fillMethod() {
        return this._fillMethod; // this._content.fillMethod;
    }
    set fillMethod(value) {
        this._fillMethod = value;
    ...
    get fillOrigin() {
        return this._fillOrigin; // this._content.fillOrigin;
    }
    set fillOrigin(value) {
        this._fillOrigin = value;
        if (this._content.spriteFrame)
    ...
    get fillClockwise() {
        return this._fillClockwise; //; this._content.fillClockwise;
    }
    set fillClockwise(value) {
        this._fillClockwise = value;
        if (this._content.spriteFrame)
    ...
    get fillAmount() {
        return this._fillAmount; // this._content.fillAmount;
    }
    set fillAmount(value) {
        this._fillAmount = value;
        if (this._content.spriteFrame)
    ...
    set icon(value) {
        if (value.startsWith("ui://")) {
            this._icon = value;
            let item = UIPackage.getItemByURL(this._icon);
            if (item) {
                let contentItem = item.getBranch();
                this.sourceWidth = contentItem.width;
                this.sourceHeight = contentItem.height;
                //this.initWidth = this.sourceWidth;
                //this.initHeight = this.sourceHeight;
                this._contentItem = contentItem = contentItem.getHighResolution();
                contentItem.load();
    ...
    updateNotAsset() {
        ...
        this._content.spriteFrame = contentItem.asset;
        this.fillMethod = this._fillMethod;
        if (this._fillMethod != 0) {
            this.fillOrigin = this._fillOrigin;
            this.fillClockwise = this._fillClockwise;
            this.fillAmount = this._fillAmount;
        }
    }
    constructFromResource(async = false, completeHandler = null) {
        var contentItem = this.packageItem.getBranch();
        ...
        let asset = contentItem.load();
        if (!asset) {
            this.packageItem.owner.addNotAssetImage(this);
        }
        if (contentItem.scale9Grid)
            this._content.type = Sprite.Type.SLICED;
        else if (contentItem.scaleByTile)
    ...
    handleVisibleChanged() {
        ...
        if (this.visible && this._alpha !== 1) {
            this._uiOpacity.opacity = this._alpha * (254 + Math.random()); //255 为了产生改变重新渲染
        }
    }
    getProp(index) {
        if (index == ObjectPropID.Color)
    ...
    setup_beforeAdd(buffer, beginPos) {
        ...
        this.fillMethod = buffer.readByte();
        if (this.fillMethod != 0) {
            this.fillOrigin = buffer.readByte();
            this.fillClockwise = buffer.readBool();
...
class GMovieClip extends GObject {
    constructor() {
        ...
        this._content.sizeMode = Sprite.SizeMode.CUSTOM;
        this._content.trim = false;
        //this._content.sizeMode = Sprite.SizeMode.RAW;
        //this._content.trim = true;
    ...
    constructFromResource(async = false, completeHandler = null) {
        ...
        let asset = contentItem.load();
        if (!asset) {
            contentItem.owner.addNotAssetMovieClip(this);
        }
        this._content.interval = contentItem.interval;
        this._content.swing = contentItem.swing;
        this._content.repeatDelay = contentItem.repeatDelay;
...
function updateScaler() {
    let size = screen.windowSize.clone();
...
class UIPackageHelper {

    static getParser(type) {
        let cache = this.parserMap[type];
        if (!cache) {
            cache = [];
            this.parserMap[type] = cache;
        }
        let parser = cache.pop();
        if (!parser) {
            ...
            switch (type) {
                ...
                case objectType.ProgressBar:
                    parser = new ProgressBarParser();
                    break;
                case objectType.Loader:
                    parser = new LoaderParser();
                    break;
                case objectType.ScrollBar:
                    parser = new ScrollBarParser();
                    break;
                case objectType.Slider:
                    parser = new SliderParser();
                    break;
                ...
                case objectType.MovieClip:
                    parser = new MovieClipParser();
                    break;
                // case 50:
                //     parser = new MirrorImageParser();
                //     break;
            }
        }
        return parser;
    }
    static recoverParser(parser) {
        let type;
        let objectType = ObjectType;
        if (parser instanceof ImageParser) {
            type = objectType.Image;
        }
        else if (parser instanceof ButtonParser) {
            type = objectType.Button;
        }
        else if (parser instanceof ComboBoxParser) {
            type = objectType.ComboBox;
        }
        else if (parser instanceof GraphParser) {
            type = objectType.Graph;
        }
        else if (parser instanceof GroupParser) {
            type = objectType.Group;
        }
        else if (parser instanceof LabelParser) {
            type = objectType.Label;
        }
        else if (parser instanceof LoaderParser) {
            type = objectType.Loader;
        }
        else if (parser instanceof TreeParser) {
            type = objectType.Tree;
        }
        else if (parser instanceof ListParser) {
            type = objectType.List;
        }
        else if (parser instanceof ProgressBarParser) {
            type = objectType.ProgressBar;
        }
        else if (parser instanceof SliderParser) {
            type = objectType.Slider;
        }
        else if (parser instanceof ScrollBarParser) {
            type = objectType.ScrollBar;
        }
        else if (parser instanceof RichTextParser) {
            type = objectType.RichText;
        }
        else if (parser instanceof TextInputParser) {
            type = objectType.InputText;
        }
        else if (parser instanceof TextParser) {
            type = objectType.Text;
        }
        else if (parser instanceof MovieClipParser) {
            type = objectType.MovieClip;
        } /* else if (parser instanceof MirrorImageParser) {
            type = 50;
        }*/
        else if (parser instanceof ComponentParser) {
        ...
        parser.completeHandler = null;
        parser.async = false;
        cache.push(parser);
    }
    static getParserByObjInstance(obj) {
        let objectType = ObjectType;
        /*if (obj instanceof GMirrorImage) {
            return this.getParser(50);
        } else */ if (obj instanceof GImage) {
            return this.getParser(objectType.Image);
        }
        else if (obj instanceof GButton) {
            return this.getParser(objectType.Button);
        }
        else if (obj instanceof GComboBox) {
            return this.getParser(objectType.ComboBox);
        }
        else if (obj instanceof GGraph) {
            return this.getParser(objectType.Graph);
        }
        else if (obj instanceof GGroup) {
            return this.getParser(objectType.Group);
        }
        else if (obj instanceof GLabel) {
            return this.getParser(objectType.Label);
        }
        else if (obj instanceof GList) {
            return this.getParser(objectType.List);
        }
        else if (obj instanceof GLoader) {
            return this.getParser(objectType.Loader);
        }
        else if (obj instanceof GTree) {
            return this.getParser(objectType.Tree);
        }
        else if (obj instanceof GProgressBar) {
            return this.getParser(objectType.ProgressBar);
        }
        else if (obj instanceof GSlider) {
            return this.getParser(objectType.Slider);
        }
        else if (obj instanceof GScrollBar) {
            return this.getParser(objectType.ScrollBar);
        }
        else if (obj instanceof GRichTextField) {
            return this.getParser(objectType.RichText);
        }
        else if (obj instanceof GTextInput) {
            return this.getParser(objectType.InputText);
        }
        else if (obj instanceof GTextField) {
            return this.getParser(objectType.Text);
        }
        else if (obj instanceof GComponent) {
            return this.getParser(objectType.Component);
        }
        else if (obj instanceof GMovieClip) {
            return this.getParser(objectType.MovieClip);
        }
    }
    // static getGearParserByIndex(gearIndex: number) {
    //     if (!this.gearParserArray) {
    //         this.gearParserArray = [
    //             new GearDisplayParser,
    //             new GearXYParser,
    //             new GearSizeParser,
    //             new GearLookParser,
    //             new GearColorParser,
    //             new GearAnimationParser,
    //             new GearTextParser,
    //             new GearIconParser,
    //             new GearDisplay2Parser,
    //             new GearFontSizeParser
    //         ]
    //     }
    //     return this.gearParserArray[gearIndex];
    // }
    static constructFromResource2(component, async = false, completeHandler = null) {
        let contentItem = component.packageItem.getBranch();
        let componentParser = UIPackageHelper.getParserByObjInstance(component);
        if (async) {
            componentParser.parse(component, contentItem, async, () => {
                UIPackageHelper.recoverParser(componentParser);
                completeHandler && completeHandler();
            });
        }
        else {
            componentParser.parse(component, contentItem);
            UIPackageHelper.recoverParser(componentParser);
        }
    }
}
UIPackageHelper.parserMap = {};
class BaseParser {
    constructor() {
        /**
         * 标记是否是异步创建UI
        ...
        this._beforeAddSep = [];
        this._beforeAddSep.push(this._readSetupBeforeAddBase);
        this._beforeAddSep.push(this._readSetupBeforeAddXY);
        this._beforeAddSep.push(this._readSetupBeforeAddSize);
        this._beforeAddSep.push(this._readSetupBeforeAddMinMax);
        this._beforeAddSep.push(this._readSetupBeforeAddScale);
        this._beforeAddSep.push(this._readSetupBeforeAddSkew);
        this._beforeAddSep.push(this._readSetupBeforeAddPivot);
        this._beforeAddSep.push(this._readSetupBeforeAddBaseOther);
        this._callBeforeAddSepIndex = -1;
        this._afterAddSep = [];
        this._afterAddSep.push(this._readSetupAfterAddBase);
        this._callAfterAddSepIndex = -1;
    }
    setup_beforeAdd(obj, buffer) {
        this._callBeforeAddSepIndex = -1;
        this._callNextBeforeAdd(obj, buffer);
    }
    setup_afterAdd(obj, buffer) {
        this._callAfterAddSepIndex = -1;
        this._callNextAfterAdd(obj, buffer);
    }
    _callNextBeforeAdd(obj, buffer) {
        ++this._callBeforeAddSepIndex;
        this._callSetupFun(this._beforeAddSep[this._callBeforeAddSepIndex], obj, buffer);
    }
    _callNextAfterAdd(obj, buffer) {
        ++this._callAfterAddSepIndex;
        this._callSetupFun(this._afterAddSep[this._callAfterAddSepIndex], obj, buffer);
    }
    _callSetupFun(fun, obj, buffer) {
        if (fun) {
            if (this.async) {
                if (game.totalTime - game.frameStartTime >= 16) {
                    GRoot.inst.timer.frameOnce(1, this, fun, [obj, buffer]);
                }
                else {
                    fun.call(this, obj, buffer);
                }
            }
            else {
                try {
                    ...
                    // 追加打印定位问题
                    console.error(`FGUI 二进制类型, 通过 component 绑定的 Class 反解析子节点出错. 请检查子组件 extends fgui.GComponent 是否正确. parent name = ${obj.name}`);
                    throw e;
                }
            }
        }
            ...
            if (this.async) {
                if (game.totalTime - game.frameStartTime >= 16) {
                    GRoot.inst.timer.frameOnce(1, this, this.completeHandler);
                }
                else {
    ...
    _readSetupBeforeAddBase(obj, buffer) {
        obj._id = buffer.readString();
        obj._name = buffer.readString();
        obj.node.name = obj._name;
        this._callNextBeforeAdd(obj, buffer);
    }
    _readSetupBeforeAddSize(obj, buffer) {
        if (buffer.readBool()) {
            obj.initWidth = buffer.readInt();
            obj.initHeight = buffer.readInt();
            obj.setSize(obj.initWidth, obj.initHeight, true);
        }
        this._callNextBeforeAdd(obj, buffer);
    }
    _readSetupBeforeAddXY(obj, buffer) {
        var f1 = buffer.readInt();
        var f2 = buffer.readInt();
        obj.setPosition(f1, f2);
        this._callNextBeforeAdd(obj, buffer);
    }
    _readSetupBeforeAddScale(obj, buffer) {
        if (buffer.readBool()) {
    ...
    _readSetupBeforeAddSkew(obj, buffer) {
        if (buffer.readBool()) {
            buffer.readFloat();
            buffer.readFloat();
            // obj.skewX(f1, f2);
    ...
    _readSetupBeforeAddPivot(obj, buffer) {
        if (buffer.readBool()) {
            let f1 = buffer.readFloat();
            let f2 = buffer.readFloat();
            obj.setPivot(f1, f2, buffer.readBool());
        }
        this._callNextBeforeAdd(obj, buffer);
    }
    _readSetupBeforeAddMinMax(obj, buffer) {
        if (buffer.readBool()) {
            obj.minWidth = buffer.readInt();
            obj.maxWidth = buffer.readInt();
            obj.minHeight = buffer.readInt();
            obj.maxHeight = buffer.readInt();
        }
        this._callNextBeforeAdd(obj, buffer);
    }
    _readSetupBeforeAddBaseOther(obj, buffer) {
        let f1;
        f1 = buffer.readFloat();
        if (f1 !== 1)
            obj.alpha = f1;
    ...
    _readSetupAfterAddBase(obj, buffer) {
        var groupId = buffer.readShort();
        if (groupId >= 0)
            obj.group = obj.parent.getChildAt(groupId);
        var cnt = buffer.readByte();
        var gearIndex;
        for (var i = 0; i < cnt; i++) {
            gearIndex = buffer.readByte();
            var gear = obj.getGear(gearIndex);
            gear.setup(buffer);
            // UIPackageHelper.getGearParserByIndex(gearIndex).setup(gear, buffer);
        }
        this._callNextAfterAdd(obj, buffer);
    }
    parse(obj, ...arg) {
...
class ComponentParser extends BaseParser {
    constructor() {
        super();
        this._afterAddSep.push(this._readAfterAddControlPage);
        this._afterAddSep.push(this._readAfterAddProp);
        this._readParseSep = [];
        this._readParseSep.push(this._parseReadSize);
        this._readParseSep.push(this._parseReadMinMax);
        this._readParseSep.push(this._parseReadPivot);
        this._readParseSep.push(this._parseReadMargin);
        this._readParseSep.push(this._parseReadOverflow);
        this._readParseSep.push(this._parseReadController);
        this._readParseSep.push(this._parseReadChildren);
        this._readParseSep.push(this._parseReadRelation);
        this._readParseSep.push(this._parseReadSetupAfterAdd);
        this._readParseSep.push(this._parseReadMaskAndOpaque);
        this._readParseSep.push(this._parseReadTransition);
        this._readParseSep.push(this._parseStepUp);
        //this._readParseSep.push(this._parseCallChildrenInitFun);
        this._callParseFunIndex = -1;
        this.countChild = 0;
        this.currentChildIndex = 0;
    }
    parse(componentObj, componentPI, async = false, completeHandler = null) {
        var buffer = new ByteBuffer(componentPI.rawData.data);
        this._callParseFunIndex = -1;
        this.async = async;
        this.completeHandler = completeHandler;
        this._callParseNext(componentObj, buffer, componentPI);
    }
    _callParseNext(componentObj, buffer, componentPI) {
        ++this._callParseFunIndex;
        let fun = this._readParseSep[this._callParseFunIndex];
        if (fun) {
            if (this.async) {
                if (game.totalTime - game.frameStartTime >= 16) {
                    GRoot.inst.timer.frameOnce(1, this, fun, [componentObj, buffer, componentPI]);
                }
                else {
                    fun.call(this, componentObj, buffer, componentPI);
                }
            }
            else {
                fun.call(this, componentObj, buffer, componentPI);
    ...
    _readAfterAddControlPage(obj, buffer) {
        //读取组件自定义属性（也就是控制器导出的属性，用于显示页面时，默认选择哪个控制器的哪个状态）
        let cnt = buffer.readUshort();
        let cc, pageId;
        for (let i = 0; i < cnt; i++) {
            cc = obj.getController(buffer.readString());
            pageId = buffer.readString();
            if (cc)
                cc.selectedPageId = pageId;
        }
        this._callNextAfterAdd(obj, buffer);
    }
    _readAfterAddProp(obj, buffer) {
        let cnt = buffer.readUshort();
        for (let i = 0; i < cnt; ++i) {
            var target = buffer.readString();
            var propertyId = buffer.readShort();
            var value = buffer.readString();
            var objProp = obj.getChildByPath(target);
            if (objProp)
                objProp.setProp(propertyId, value);
        }
        this._callNextAfterAdd(obj, buffer);
    }
    _parseReadSize(componentObj, buffer, componentPI) {
        componentObj.sourceWidth = componentPI.width;
        componentObj.sourceHeight = componentPI.height;
        componentObj.initWidth = componentObj.sourceWidth;
        componentObj.initHeight = componentObj.sourceHeight;
        componentObj.setSize(componentObj.sourceWidth, componentObj.sourceHeight);
        this._callParseNext(componentObj, buffer, componentPI);
    }
    _parseReadMinMax(componentObj, buffer, componentPI) {
        if (buffer.readBool()) {
            componentObj.minWidth = buffer.readInt();
            componentObj.maxWidth = buffer.readInt();
            componentObj.minHeight = buffer.readInt();
            componentObj.maxHeight = buffer.readInt();
        }
        this._callParseNext(componentObj, buffer, componentPI);
    }
    _parseReadPivot(componentObj, buffer, componentPI) {
        if (buffer.readBool()) {
            let f1 = buffer.readFloat();
            let f2 = buffer.readFloat();
            componentObj.setPivot(f1, f2, buffer.readBool());
        }
        this._callParseNext(componentObj, buffer, componentPI);
    }
    _parseReadMargin(componentObj, buffer, componentPI) {
        if (buffer.readBool()) {
            componentObj.margin.top = buffer.readInt();
            componentObj.margin.bottom = buffer.readInt();
            componentObj.margin.left = buffer.readInt();
            componentObj.margin.right = buffer.readInt();
        }
        this._callParseNext(componentObj, buffer, componentPI);
    }
    _parseReadOverflow(componentObj, buffer, componentPI) {
        var overflow = buffer.readByte();
        if (overflow === OverflowType.Scroll) {
            overflowParser.parse(componentObj, buffer);
        }
        else {
            //@ts-ignore
            componentObj.setupOverflow(overflow);
        }
        this._callParseNext(componentObj, buffer, componentPI);
    }
    _parseReadController(componentObj, buffer, componentPI) {
        //分析控制器
        var controllerCount = buffer.readUshort();
        for (var i = 0; i < controllerCount; i++) {
            var controller = new Controller();
            componentObj._controllers.push(controller);
            controller.parent = componentObj;
            controllerParser.setup(controller, buffer);
        }
        this._callParseNext(componentObj, buffer, componentPI);
    }
    _parseReadChildren(componentObj, buffer, componentPI) {
        componentObj._buildingDisplayList = true;
        let childCount = buffer.readUshort();
        this.countChild = childCount;
        this.currentChildIndex = 0;
        if (this.async) {
            let completeHandler = () => {
                //这里延迟是要避免JS的调用堆栈太深，造成 Maximum call stack size exceeded
                GRoot.inst.timer.callLater(this, this._parseNextChild1, [componentObj, buffer, componentPI, completeHandler]);
            };
            this._parseNextChild1(componentObj, buffer, componentPI, completeHandler);
        }
        else {
            for (let i = 0; i <= childCount; ++i) {
                this._parseNextChild1(componentObj, buffer, componentPI, null);
            }
        }
    }
    _parseNextChild1(componentObj, buffer, componentPI, completeHandler) {
        ++this.currentChildIndex;
        if (this.currentChildIndex > this.countChild) {
            if (completeHandler) {
            ...
            this._parseNextChild2(componentObj, buffer, componentPI, completeHandler);
        }
    }
    _parseNextChild2(componentObj, buffer, componentPI, completeHandler) {
        let child, src, pkgId, type, pi, pkg, bindName;
        type = buffer.readByte();
        src = null;
        pkgId = null;
        if (type === ObjectType.Component || type === ObjectType.Image) {
            src = buffer.readString();
            pkgId = buffer.readString();
        }
        bindName = buffer.readBool();
        pi = null;
        if (src) {
            pkg = null;
            if (pkgId) {
                ...
                // if(!pkg){
                //     throw new Error("fgui包资源还没有加载就使用:"+pkgId);
                // }
            }
            else {
                pkg = componentPI.owner;
            }
            pi = pkg ? pkg.getItemById(src) : null;
        }
        child = pi ? UIObjectFactory.newObject(pi) : UIObjectFactory.newObject(type);
        if (pi) {
            if (completeHandler) {
                //@ts-ignore
                child.constructFromResource(true, this._parseNextChild3.bind(this, buffer, componentObj, child, bindName, completeHandler));
            }
            else {
                child.constructFromResource();
                this._parseNextChild3(buffer, componentObj, child, bindName);
            }
        }
        else {
            this._parseNextChild3(buffer, componentObj, child, bindName, completeHandler);
        }
    }
    _parseNextChild3(buffer, componentObj, child, bindName, completeHandler = null) {
        child._parent = componentObj;
        child.node.parent = componentObj._container;
        componentObj._children.push(child);
        let parser = UIPackageHelper.getParserByObjInstance(child);
        child._underConstruct = true;
        if (completeHandler) {
            parser.async = true;
            parser.completeHandler = () => {
                UIPackageHelper.recoverParser(parser);
                child._parent = componentObj;
                if (bindName) {
                    //@ts-ignore
                    componentObj[child._name] = child;
                }
                completeHandler();
            };
            parser.setup_beforeAdd(child, buffer);
        }
        else {
            parser.setup_beforeAdd(child, buffer);
            child._parent = componentObj;
    ...
    _parseReadRelation(componentObj, buffer, componentPI) {
        relationParser.parse(componentObj, buffer, true);
        let childCount = componentObj._children.length;
        for (let i = 0; i < childCount; ++i) {
            relationParser.parse(componentObj._children[i], buffer, false);
        }
        this._callParseNext(componentObj, buffer, componentPI);
    }
    _parseReadSetupAfterAdd(componentObj, buffer, componentPI) {
        let childCount = componentObj._children.length;
        this.currentChildIndex = -1;
        if (this.async) {
            let completeHandler = () => {
                //这里延迟是要避免JS的调用堆栈太深，造成 Maximum call stack size exceeded
                GRoot.inst.timer.callLater(this, this._callChildStepAfterAdd1, [componentObj, buffer, componentPI, completeHandler]);
            };
            this._callChildStepAfterAdd1(componentObj, buffer, componentPI, completeHandler);
        }
        else {
            for (let i = 0; i <= childCount; i++) {
                this._callChildStepAfterAdd1(componentObj, buffer, componentPI);
            }
        }
    }
    _callChildStepAfterAdd1(componentObj, buffer, componentPI, completeHandler = null) {
        ++this.currentChildIndex;
        let child = componentObj._children[this.currentChildIndex];
        if (child) {
            this._callChildStepAfterAdd2(child, buffer, completeHandler);
            child._underConstruct = false;
        }
        else {
            if (completeHandler) {
    ...
    _callChildStepAfterAdd2(child, buffer, completeHandler) {
        let parser = UIPackageHelper.getParserByObjInstance(child);
        if (completeHandler) {
            parser.async = true;
            parser.completeHandler = () => {
            ...
            parser.setup_afterAdd(child, buffer);
        }
        else {
            parser.setup_afterAdd(child, buffer);
            UIPackageHelper.recoverParser(parser);
    ...
    _parseReadMaskAndOpaque(componentObj, buffer, componentPI) {
        componentObj.opaque = buffer.readBool();
        var maskIndex = buffer.readShort();
        if (maskIndex !== -1) {
            componentObj.setMask(componentObj.getChildAt(maskIndex), buffer.readBool());
        }
        this._callParseNext(componentObj, buffer, componentPI);
    }
    _parseReadTransition(componentObj, buffer, componentPI) {
        var transitionCount = buffer.readByte();
        for (let i = 0; i < transitionCount; ++i) {
            var trans = new Transition(componentObj);
            trans.setup(buffer);
            // transitionParser.setup(trans, buffer);
            componentObj._transitions.push(trans);
        }
        this._callParseNext(componentObj, buffer, componentPI);
    }
    _parseStepUp(componentObj, buffer, componentPI) {
        componentObj.applyAllControllers();
        componentObj._buildingDisplayList = false;
        componentObj._underConstruct = false;
        //@ts-ignore
        componentObj.buildNativeDisplayList();
        componentObj.setBoundsChangedFlag();
        if (componentPI.objectType !== ObjectType.Component)
            this.constructExtension(componentObj, buffer);
        //@ts-ignore
        componentObj.onConstruct();
        this._callParseNext(componentObj, buffer, componentPI);
    }

    constructExtension(gObj, buffer) {
    }
}
class ButtonParser extends ComponentParser {
    constructor() {
        super();
        this._afterAddSep.push(this._readSetupAfterAddButton);
    }
    _readSetupAfterAddButton(obj, buffer) {
        if (!buffer.readBool()) {
            this._callNextAfterAdd(obj, buffer);
            return;
        }
        obj.setup_afterAdd(buffer);
        this._callNextAfterAdd(obj, buffer);
    }
    constructExtension(obj, buffer) {
        //@ts-ignore
        obj.constructExtension(buffer);
    }
}
class GraphParser extends BaseParser {
    constructor() {
        super();
        this._beforeAddSep.push(this._readStepBeforeAdd);
    }
    _readStepBeforeAdd(obj, buffer) {
        obj.setup_beforeAdd(buffer);
        this._callNextBeforeAdd(obj, buffer);
    }
}
class GroupParser extends BaseParser {
    constructor() {
        super();
        this._beforeAddSep.push(this._readStepBeforeAdd);
        this._afterAddSep.push(this._readSetupAfterAddGroup);
    }
    _readStepBeforeAdd(obj, buffer) {
        obj.setup_beforeAdd(buffer);
        this._callNextBeforeAdd(obj, buffer);
    }
    _readSetupAfterAddGroup(obj, buffer) {
        if (!obj.visible)
            obj.handleVisibleChanged();
        this._callNextAfterAdd(obj, buffer);
    }
}
class ImageParser extends BaseParser {
    ...
    _readStepBeforeAdd(obj, buffer) {
        if (buffer.readBool())
            obj.icon = "ui/" + buffer.readString();
        obj.setup_beforeAdd(buffer);
        this._callNextBeforeAdd(obj, buffer);
    }
}
class MovieClipParser extends BaseParser {
    constructor() {
        super();
        this._beforeAddSep.push(this._readStepBeforeAdd);
    }
...
// class MirrorImageParser extends BaseParser {
//     constructor() {
//         super();
//         this._beforeAddSep.push(this._readStepBeforeAdd);
//     }
//     _readStepBeforeAdd(obj: any, buffer: ByteBuffer) {
//         obj.mirrorType = buffer.readByte();
//         let pkgId = buffer.readString();
//         let src = buffer.readString();
//         let pkg = UIPackage.getById(pkgId);
//         if (pkg) {
//             let item = pkg.getItemById(src);
//             obj.packageItem = item;
//             obj._contentItem = obj.packageItem.getBranch();
//             obj.sourceWidth = obj._contentItem.width;
//             obj.sourceHeight = obj._contentItem.height;
//             obj.initWidth = obj.sourceWidth;
//             obj.initHeight = obj.sourceHeight;
//             obj._contentItem = obj._contentItem.getHighResolution();
//             obj._contentItem.load();
//             obj.image.scale9Grid = obj._contentItem.scale9Grid;
//             obj.image.scaleByTile = obj._contentItem.scaleByTile;
//             obj.image.tileGridIndice = obj._contentItem.tileGridIndice;
//             obj.image.texture = obj._contentItem.texture;
//         }
//         this._callNextBeforeAdd(obj, buffer);
//     }
// }
class LabelParser extends ComponentParser {
    constructor() {
        super();
        this._afterAddSep.push(this._readSetupAfterAddLabel);
    }
    _readSetupAfterAddLabel(obj, buffer) {
        if (!buffer.readBool()) {
            this._callNextAfterAdd(obj, buffer);
            return;
        }
...
class ListParser extends BaseParser {
    constructor() {
        super();
        this._beforeAddSep.push(this._readStepBeforeAddList);
        this._afterAddSep.push(this._readSetupAfterAddList);
    }
    _readStepBeforeAddList(obj, buffer) {
        obj.setup_beforeAdd(buffer);
        this._callNextBeforeAdd(obj, buffer);
    }
    _readSetupAfterAddList(obj, buffer) {
        obj.setup_afterAdd(buffer);
        this._callNextAfterAdd(obj, buffer);
    }
}
class TreeParser extends ListParser {
    constructor() {
        super();
        this._beforeAddSep.push(this._readStepBeforeAddTree);
    }
    _readStepBeforeAddTree(obj, buffer) {
        obj.setup_beforeAdd(buffer);
        this._callNextBeforeAdd(obj, buffer);
    }
}
...
class ProgressBarParser extends ComponentParser {
    constructor() {
        super();
        this._afterAddSep.push(this._readSetupAfterAddProgressBar);
    }
    _readSetupAfterAddProgressBar(obj, buffer) {
        if (!buffer.readBool()) {
            this._callNextAfterAdd(obj, buffer);
            return;
        }
...
class ScrollBarParser extends ComponentParser {
    constructExtension(obj, buffer) {
        //@ts-ignore
        obj.constructExtension(buffer);
    }
}
class SliderParser extends ComponentParser {
    constructor() {
        super();
        this._afterAddSep.push(this._readSetupAfterAddSlider);
    }
    _readSetupAfterAddSlider(obj, buffer) {
        if (!buffer.readBool()) {
            this._callNextAfterAdd(obj, buffer);
            return;
        }
...
class TextParser extends BaseParser {
    constructor() {
        super();
        this._beforeAddSep.push(this._readStepBeforeAddText);
        this._afterAddSep.push(this._readSetupAfterAddText);
    }
    _readStepBeforeAddText(obj, buffer) {
        obj.setup_beforeAdd(buffer);
        this._callNextBeforeAdd(obj, buffer);
    }
    _readSetupAfterAddText(obj, buffer) {
        obj.setup_afterAdd(buffer);
        this._callNextAfterAdd(obj, buffer);
    }
}
class TextInputParser extends TextParser {
}
class RichTextParser extends TextParser {
}
class ComboBoxParser extends ComponentParser {
    constructor() {
        super();
        this._afterAddSep.push(this._readSetupAfterAddCombobox);
    }
    _readSetupAfterAddCombobox(obj, buffer) {
        if (!buffer.readBool()) {
            this._callNextAfterAdd(obj, buffer);
            return;
        }
...
var controllerParser = {
    setup: function (controller, buffer) {
        controller.setup(buffer);
    }
};
var relationParser = {
    parse: function (obj, buffer, parentToChild) {
        var relation = obj.relations;
        relation.setup(buffer, parentToChild);
    }
};
var overflowParser = {
    parse: function (obj, buffer) {
        var scrollPane = obj._scrollPane = obj.node.addComponent(ScrollPane);
...
//     setup: function (obj: Transition, buffer: ByteBuffer) {
//         obj.name = buffer.readString();
//         //@ts-ignore
//         obj._options = buffer.readByte();
//         //@ts-ignore
//         obj._autoPlay = buffer.readBool();
//         //@ts-ignore
//         obj._autoPlayTimes = buffer.readUint32();
//         //@ts-ignore
//         obj._autoPlayDelay = buffer.readFloat();
//         var cnt = buffer.readByte();
//         for (var i = 0; i < cnt; i++) {
...
//             else
//                 //@ts-ignore
//                 item.targetId = obj._owner.getChildAt(targetId).id;
//             item.label = buffer.readString();
//             if (buffer.readBool()) {
//                 item.tweenConfig = new TweenConfig();
//                 item.tweenConfig.duration = buffer.readFloat();
//                 //@ts-ignore
//                 if (item.time + item.tweenConfig.duration > obj._totalDuration)
//                     //@ts-ignore
//                     obj._totalDuration = item.time + item.tweenConfig.duration;
//                 item.tweenConfig.easeType = buffer.readByte();
//                 item.tweenConfig.repeat = buffer.readInt();
//                 item.tweenConfig.yoyo = buffer.readBool();
//                 // item.tweenConfig.endLabel = buffer.readString();
...
//             } else {
//                 //@ts-ignore
//                 if (item.time > obj._totalDuration)
//                     //@ts-ignore
//                     obj._totalDuration = item.time;
//                 this.decodeValue(item, buffer, item.value);
//             }
//         }
//     },
//     decodeValue: function (item: Item, buffer: ByteBuffer, value: TValue) {
//         switch (item.type) {
//             case ActionType.XY:
...
//                 value.f1 = buffer.readColor()._val;
//                 break;
...
//                 value.sound = buffer.readString();
//                 value.volume = buffer.readFloat();
//                 break;
//             case ActionType.Transition:
//                 value.transName = buffer.readString();
...
//                 value.text = buffer.readString();
//                 break;
//         }
//     }
// }
// class BaseGearParser {
//     setup(obj: GearBase, buffer: ByteBuffer) {
//         obj.setup(buffer);
//     }
//     addStatus(obj: GearBase, pageId: string, buffer: ByteBuffer) {
//     }
//     addExtStatus(obj: GearBase, pageId: string, buffer: ByteBuffer) {
//     }
// }
// class GearColorParser extends BaseGearParser {
//     addStatus(obj: GearColor, pageId: string, buffer: ByteBuffer) {
//         //@ts-ignore
//         obj.addStatus(pageId, buffer);
//     }
// }
// class GearDisplayParser extends BaseGearParser {
// }
// class GearDisplay2Parser extends BaseGearParser {
// }
// class GearFontSizeParser extends BaseGearParser {
//     addStatus(obj: GearFontSize, pageId: string, buffer: ByteBuffer) {
//         //@ts-ignore
//         obj.addStatus(pageId, buffer);
//     }
// }
// class GearIconParser extends BaseGearParser {
//     addStatus(obj: GearIcon, pageId: string, buffer: ByteBuffer) {
//         //@ts-ignore
//         obj.addStatus(pageId, buffer);
//     }
// }
// class GearLookParser extends BaseGearParser {
//     addStatus(obj: GearLook, pageId: string, buffer: ByteBuffer) {
//         //@ts-ignore
//         obj.addStatus(pageId, buffer);
//     }
// }
// class GearSizeParser extends BaseGearParser {
//     addStatus(obj: GearSize, pageId: string, buffer: ByteBuffer) {
//         //@ts-ignore
//         obj.addStatus(pageId, buffer);
//     }
// }
// class GearTextParser extends BaseGearParser {
//     addStatus(obj: GearText, pageId: string, buffer: ByteBuffer) {
//         //@ts-ignore
//         obj.addStatus(pageId, buffer);
//     }
// }
// class GearXYParser extends BaseGearParser {
//     addStatus(obj: GearXY, pageId: string, buffer: ByteBuffer) {
//         //@ts-ignore
//         obj.addStatus(pageId, buffer);
//     }
//     addExtStatus(obj: GearXY, pageId: string, buffer: ByteBuffer) {
//         //@ts-ignore
//         obj.addExtStatus(pageId, buffer);
//     }
// }
// class GearAnimationParser extends BaseGearParser {
//     addStatus(obj: GearAnimation, pageId: string, buffer: ByteBuffer) {
//         //@ts-ignore
//         obj.addStatus(pageId, buffer);
//     }
// }
...
class ByteBuffer {
    ...
    readSArray(cnt) {
        ...
        for (var i = 0; i < cnt; i++)
            ret[i] = this.readString(); //this.readS();
        return ret;
    }
    writeS(value) {
        var index = this.readUshort();
...
class UIPackage {
    ...
    static addPackage(path) {
        ...
        //新增zip解码
        let byt = new Uint8Array(buffer);
        let z = new Zlib.Inflate(byt);
        buffer = z.decompress();
        //
    ...
    static createObjectAsync(pkgName, resName, compFun) {
        var pkg = UIPackage.getByName(pkgName);
        if (pkg)
            pkg.createObjectAsync(pkg.getItemByName(resName), compFun);
        else
            compFun && compFun(null);
    }
    ...
    loadPackage(buffer, path) {
        ...
        this._id = buffer.readString();
        this._name = buffer.readString();
        let itemCount = buffer.readUshort();
        let pi;
        let pos = path.lastIndexOf('/');
        let shortPath = pos === -1 ? "" : path.substr(0, pos + 1);
        for (let i = 0; i < itemCount; ++i) {
            ...
            pi.owner = this;
            pi.type = buffer.readByte();
            pi.id = buffer.readString();
            pi.name = PathUtils.mainFileName(buffer.readString());
            switch (pi.type) {
                case PackageItemType.Image: {
                    pi.objectType = ObjectType.Image;
                    ...
                    let atlasName = buffer.readString();
                    let atlasPI = this._itemsById[atlasName];
                    //所在图集
                    if (!atlasPI) {
                        atlasPI = new PackageItem();
                        atlasPI.owner = this;
                        atlasPI.type = PackageItemType.Atlas;
                        atlasPI.objectType = ObjectType.Image;
                        atlasPI.id = atlasName;
                        atlasPI.name = atlasName;
                        atlasPI.file = shortPath + PathUtils.mainFileName(atlasName);
                        this._items.push(atlasPI);
                        this._itemsById[atlasPI.id] = atlasPI;
                    }
                    sprite.atlas = atlasPI;
                    ...
                    pi.width = sprite.originalSize.x;
                    pi.height = sprite.originalSize.y;
                    this._sprites[pi.id] = sprite;
                    break;
                }
                case PackageItemType.Component: {
                    pi.width = buffer.readUint();
                    pi.height = buffer.readUint();
    ...
    dispose() {
        this._notAssetTextField = null;
        this._notAssetImage = null;
        this._notAssetMovieClip = null;
        var cnt = this._items.length;
        for (var i = 0; i < cnt; i++) {
    ...
    createObjectAsync(pi, compFun) {
        let waitForCreate = this._waitForCreate;
        if (!waitForCreate) {
            waitForCreate = this._waitForCreate = [];
        }
        waitForCreate.push(pi);
        waitForCreate.push(compFun);
        this.nextCreateAsync();
    }
    nextCreateAsync() {
        if (this._isCreateing) {
            return;
        }
        let waitForCreate = this._waitForCreate;
        if (waitForCreate.length === 0) {
            return;
        }
        this._isCreateing = true;
        let pi = waitForCreate.shift();
        let compFun = waitForCreate.shift();
        var view = Decls.UIObjectFactory.newObject(pi);
        var parser = UIPackageHelper.getParser(pi.objectType);
        parser.parse(view, pi, true, () => {
            UIPackageHelper.recoverParser(parser);
            //fgui.callOnInit(view);
            compFun && compFun(view);
            GRoot.inst.timer.callLater(this, this.callLaterNextCreateAsync);
        });
    }
    callLaterNextCreateAsync() {
        this._isCreateing = false;
        this.nextCreateAsync();
    }
    internalCreateObject(item, userClass) {
        ...
        var parser = UIPackageHelper.getParser(item.objectType);
        parser.parse(g, item);
        UIPackageHelper.recoverParser(parser);
        constructingDepth.n--;
        return g;
            ...
            case PackageItemType.Image:
                if (!item.decoded) {
                    ...
                    if (sprite) {
                        ...
                        if (atlasTexture) {
                            atlasTexture.setWrapMode(Texture2D.WrapMode.CLAMP_TO_EDGE, Texture2D.WrapMode.CLAMP_TO_EDGE);
    ...
    updateNotAssetByAtlas() {
        ...
        this._items.forEach(item => {
            ...
            else if (item.type == PackageItemType.Image && !item.asset) {
                item.decoded = false;
                item.load();
            }
            else if (item.type == PackageItemType.MovieClip && !item.asset) {
                item.decoded = false;
                item.load();
            }
            else if (item.type == PackageItemType.Font && !item.asset) {
                item.decoded = false;
                item.load();
            }
        });
        (_a = this._notAssetTextField) === null || _a === void 0 ? void 0 : _a.forEach(item => {
            !item.isDisposed && item.updateNotAsset();
        });
        (_b = this._notAssetImage) === null || _b === void 0 ? void 0 : _b.forEach(item => {
            !item.isDisposed && item.updateNotAsset();
        });
        (_c = this._notAssetMovieClip) === null || _c === void 0 ? void 0 : _c.forEach(item => {
            !item.isDisposed && item.updateNotAsset();
        });
        this._notAssetTextField = null;
        this._notAssetImage = null;
    ...
    addNotAssetTextField(item) {
        if (!this._notAssetTextField) {
            this._notAssetTextField = [];
        }
        this._notAssetTextField.push(item);
    }
    addNotAssetImage(item) {
        if (!this._notAssetImage) {
            this._notAssetImage = [];
        }
        this._notAssetImage.push(item);
    }
    addNotAssetMovieClip(item) {
        if (!this._notAssetMovieClip) {
            this._notAssetMovieClip = [];
        }
        this._notAssetMovieClip.push(item);
    }
    loadMovieClip(item) {
        var buffer = item.rawData;
        buffer.seek(0, 0);
    ...
    loadFont(item) {
        ...
        for (var i = 0; i < cnt; i++) {
            ...
            let rect = new Rect();
            bg.rect = rect;
            var img = buffer.readString(); //buffer.readS();
            rect.x = 0; //buffer.readInt();
            rect.y = 0; //buffer.readInt();
            bg.xOffset = buffer.readInt();
            bg.yOffset = buffer.readInt();
            rect.width = 0; //buffer.readInt();
            rect.height = 0; //buffer.readInt();
            bg.xAdvance = buffer.readInt();
            bg.channel = -1; //buffer.readByte();
            if (bg.channel == 1)
                bg.channel = 3;
...
class ControllerAction {
    ...
    setup(buffer) {
        var cnt;
        var i;
        cnt = buffer.readByte(); //buffer.readShort();
        this.fromPage = [];
        for (i = 0; i < cnt; i++)
            this.fromPage[i] = buffer.readString(); //buffer.readS();
        cnt = buffer.readByte(); //buffer.readShort();
        this.toPage = [];
        for (i = 0; i < cnt; i++)
            this.toPage[i] = buffer.readString(); //buffer.readS();
    }
}
...
class ChangePageAction extends ControllerAction {
    ...
    setup(buffer) {
        super.setup(buffer);
        this.objectId = buffer.readString(); //buffer.readS();
        this.controllerName = buffer.readString(); //buffer.readS();
        this.targetPage = buffer.readString(); //buffer.readS();
    }
}
...
class PlayTransitionAction extends ControllerAction {
    ...
    setup(buffer) {
        super.setup(buffer);
        this.transitionName = buffer.readString(); //buffer.readS();
...
class Controller extends EventTarget {
    ...
    setup(buffer) {
        ...
        // buffer.seek(beginPos, 0);
        this.name = buffer.readString(); //buffer.readS();
        if (buffer.readBool())
        ...
        var cnt = buffer.readUshort(); //buffer.readShort();
        for (i = 0; i < cnt; i++) {
            this._pageIds.push(buffer.readString()); //buffer.readS();
            this._pageNames.push(buffer.readString()); //buffer.readS();
        }
        var homePageIndex = 0;
        // if (buffer.version >= 2) {
        ...
        switch (homePageType) {
            case 1:
                homePageIndex = buffer.readUshort(); //buffer.readShort();
                break;
            case 2:
                ...
                break;
            case 3:
                homePageIndex = this._pageNames.indexOf(UIPackage.getVar(buffer.readString() /*buffer.readS()*/));
                if (homePageIndex == -1)
                    homePageIndex = 0;
                break;
        }
...
class ScrollPane extends Component {
    ...
    setup(buffer) {
        ...
        var vtScrollBarRes = buffer.readString(); //buffer.readS();
        var hzScrollBarRes = buffer.readString(); //buffer.readS();
        var headerRes = buffer.readString(); //buffer.readS();
        var footerRes = buffer.readString(); //buffer.readS();
        if ((flags & 1) !== 0)
    ...
    refresh(dt) {
        if (!this._owner?.node?.isValid) return;
...
class Transition {
    ...
    setup(buffer) {
        this.name = buffer.readString(); //buffer.readS();
        this._options = buffer.readByte(); //buffer.readInt();
        this._autoPlay = buffer.readBool();
        this._autoPlayTimes = buffer.readUint(); //buffer.readInt();
        this._autoPlayDelay = buffer.readFloat();
        var cnt = buffer.readByte(); //buffer.readShort();
        for (var i = 0; i < cnt; i++) {
            ...
            item.label = buffer.readString(); //buffer.readS();
            if (buffer.readBool()) {
                // buffer.seek(curPos, 1);
                item.tweenConfig = new TweenConfig();
                item.tweenConfig.duration = buffer.readFloat();
    ...
    decodeValue(item, buffer, value) {
        switch (item.type) {
            ...
            case ActionType.Visible:
                ...
                break;
            case ActionType.Sound:
                value.sound = buffer.readString(); //buffer.readS();
                value.volume = buffer.readFloat();
                break;
            case ActionType.Transition:
                value.transName = buffer.readString(); //buffer.readS();
                value.playTimes = buffer.readInt();
                break;
            case ActionType.Shake:
                value.amplitude = buffer.readFloat();
            ...
            case ActionType.Text:
            case ActionType.Icon:
                value.text = buffer.readString(); //buffer.readS();
                break;
        }
    }
...
class GComponent extends GObject {
    ...
    getChildAt(index, classType) {
        if (index >= 0 && index < this.numChildren) {
            ...
            console.error(`[FGUI Glist] invalid child index. index = ${index}, numChildrent = ${this.numChildren}, classType = ${classType}`);
            return null;
        }
    }
    ...
    onMaskReady() {
        ...
        if (this._maskContent instanceof GImage) {
            ...
            let spr = this._customMask.getComponent(Sprite);
            spr.spriteFrame = this._maskContent._content.spriteFrame;
        }
    ...
    constructFromResource(async, completeHandler) {
        // this.constructFromResource2(null, 0);
        UIPackageHelper.constructFromResource2(this, async, completeHandler);
    }
...
class GTextField extends GObject {
    ...
    set font(value) {
        if (this._font != value || !value) {
            ...
            if (newFont.startsWith("ui://")) {
                ...
                if (!newFont) {
                    pi.owner.addNotAssetTextField(this);
                }
            }
    ...
    markSizeChanged() {
        if (this._underConstruct || !isValid(this._node)) //fix：兼容清理报错
            return;
    ...
    handleSizeChanged() {
        if (this._updatingSize || !isValid(this._node)) //fix：兼容清理报错
            return;
    ...
    setup_beforeAdd(buffer, beginPos) {
        ...
        var strokeShadowTag = buffer.readByte();
        if (strokeShadowTag % 2) {
        ...
        if (strokeShadowTag >= 2) {
            ...
            let f1 = buffer.readShort();
            let f2 = buffer.readShort();
    ...
    setup_afterAdd(buffer, beginPos) {
        ...
        var str = buffer.readString(); //buffer.readS();
        if (str)
            this.text = str;
    }
}
...
class GRichTextField extends GTextField {
    ...
    updateText() {
        ...
        if (this._underline)
            text2 = "<u>" + text2 + "</u>";
        if (this._stroke > 0)
            text2 = `<outline color=${this._strokeColor.toCSS("#rrggbb")} width=${this._stroke}>` + text2 + "</outline>";
        let c = this._color;
        if (this._grayed)
            c = toGrayedColor(c);
...
class InputProcessor extends Component {
    ...
    cancelAllTouches() {
        let cnt = this._touches.length;
        for (let i = 0; i < cnt; i++) {
            /**@see InputProcessor.touchCancelHandler*/
            let ti = this._touches[i];
    ...
    touchBeginHandler(evt) {
        ...
        evt.propagationStopped = evt2.propagationStopped; //1.解决cocosUI和fgui混用点击事件使用问题
        return true;
    }
    ...
    touchEndHandler(evt) {
        ...
        this.setEnd(ti);
        let evt2 = this.getEvent(ti, ti.target, Event.TOUCH_END, false);
        let tempPropagationStopped = false; //2.解决cocosUI和fgui混用点击事件使用问题
        let cnt = ti.touchMonitors.length;
        for (let i = 0; i < cnt; i++) {
            let mm = ti.touchMonitors[i];
            if (mm == ti.target || mm.node == null || !mm.node.activeInHierarchy
            ...
            mm.node.dispatchEvent(evt2);
            tempPropagationStopped = evt2.propagationStopped; //3.解决cocosUI和fgui混用点击事件使用问题
        }
        ti.touchMonitors.length = 0;
        if (ti.target && ti.target.node) {
            ...
            evt2.bubbles = true;
            ti.target.node.dispatchEvent(evt2);
            tempPropagationStopped = evt2.propagationStopped; //4.解决cocosUI和fgui混用点击事件使用问题
        }
        returnEvent(evt2);
        ...
        if (ti.target) {
            ...
            ti.target.node.dispatchEvent(evt2);
            tempPropagationStopped = evt2.propagationStopped; //5.解决cocosUI和fgui混用点击事件使用问题
            returnEvent(evt2);
        }
        ...
        ti.touchId = -1;
        ti.button = -1;
        evt.propagationStopped = tempPropagationStopped; //6.解决cocosUI和fgui混用点击事件使用问题
    }
...
class GRoot extends GComponent {
    ...
    static create() {
        ...
        GRoot._inst.maxWidth = 750;
        //GRoot._inst.maxHeight = 1626;

        director.getScene().getChildByPath('Canvas/UI').addChild(GRoot._inst.node);
...
class GTextInput extends GTextField {
    ...
    setup_beforeAdd(buffer, beginPos) {
        ...
        str = buffer.readString(); //buffer.readS();
        if (str)
            this.restrict = str;
        var iv = buffer.readUint(); //buffer.readInt();
        if (iv !== 0)
            this.maxLength = iv;
        iv = buffer.readByte(); //buffer.readInt();
        if (buffer.readBool())
            this.password = true;
        //同步一下对齐方式
        if (this._editBox.placeholderLabel) {
...
class GLoader extends GObject {
    constructor() {
        ...
        this._content.sizeMode = Sprite.SizeMode.RAW;//Sprite.SizeMode.CUSTOM; //RAW
        this._content.trim = false;//true;
        this._content.setPlaySettings();
    }
    dispose() {
    ...
    set icon(value) {
        if (!!value && value.startsWith("unpack")) {
            value = "ui/" + value.replace(path.extname(value), ""); //去掉后缀名
        }
        this.url = value;
    }
    get align() {
        ...
        if (this._contentItem) {
            ...
            if (this._contentItem.type == PackageItemType.Image) {
                if (!this._contentItem.asset) {
                    this.setErrorState();
                    this._contentItem.owner.addNotAssetImage(this);
                }
                else {
                    this._content.spriteFrame = this._contentItem.asset;
                    if (this._content.fillMethod == 0) {
    ...
    setup_beforeAdd(buffer, beginPos) {
        ...
        if (!!url && url.startsWith("unpack")) {
            url = "ui/" + url.replace(path.extname(url), ""); //去掉后缀名
        }
    ...
    setup_afterAdd(buffer, beginPos) {
        ...
        if (buffer.readBool()) {
            ...
            if (input instanceof GTextInput) {
                ...
                if (str)
                    input.restrict = str;
                iv = buffer.readUint(); //buffer.readInt();
                if (iv != 0)
                    input.maxLength = iv;
                iv = buffer.readByte(); //buffer.readInt();
                if (buffer.readBool())
...
class GButton extends GComponent {
    ...
    setup_afterAdd(buffer, beginPos) {
        ...
        if (buffer.readBool())
            this.titleColor = buffer.readColor();
        iv = buffer.readUshort(); //buffer.readInt();
        if (iv != 0)
            this.titleFontSize = iv;
        iv = buffer.readShort();
        if (iv >= 0)
            this._relatedController = this.parent.getControllerAt(iv);
        this._relatedPageId = buffer.readString(); //buffer.readS();
        str = buffer.readString(); //buffer.readS();
        if (str)
            this._sound = str;
        if (buffer.readBool())
...
class GList extends GComponent {
    ...
    setup_beforeAdd(buffer, beginPos) {
        ...
        if (overflow == OverflowType.Scroll) {
            ...
            this.setupScroll(buffer);
            if (this._scrollPane) {
                this._scrollPane._container.__isInList = true;
            }
            // buffer.position = savedPos;
        }
        else
        ...
        this._defaultItem = buffer.readString(); //buffer.readS();
    ...
    setupItem(buffer, obj) {
        ...
        if (obj instanceof GComponent) {
            ...
            if (buffer.version >= 2) {
                cnt = buffer.readShort();
                for (i = 0; i < cnt; i++) {
                    var target: string =  buffer.readString();//buffer.readS();
                    var propertyId: number = buffer.readShort();
                    var value: String =  buffer.readString();//buffer.readS();
...
class GComboBox extends GComponent {
    ...
    setup_afterAdd(buffer, beginPos) {
        ...
        for (i = 0; i < itemCount; i++) {
            // nextPos = buffer.readShort();
            // nextPos += buffer.position;
            this._items[i] = buffer.readString(); //buffer.readS();
            this._values[i] = buffer.readString(); //buffer.readS();
            str = buffer.readString(); //buffer.readS();
            if (str) {
                if (this._icons == null)
...
class GSlider extends GComponent {
    ...
    updateWithPercent(percent, manual) {
        ...
        if (!this._reverse) {
            ...
            if (this._barObjectV)
                this._barObjectV.height = Math.round(fullHeight * (1 - percent));
        }
        else {
            if (this._barObjectH) {
                this._barObjectH.width = Math.round(fullWidth * percent);
    ...
    constructExtension(buffer) {
        ...
        if (this._gripObject) {
            ...
            this._gripObject.on(Event.TOUCH_END, this.onGripTouchEnd, this);
        }
...
export { AlignType, AsyncOperation, AutoSizeType, BlendMode, ButtonMode, ByteBuffer, ChildrenRenderOrder, Controller, DragDropManager, EaseType, Event, FillMethod, FillOrigin, FlipType, GButton, GComboBox, GComponent, GearAnimation, GearBase, GearColor, GearDisplay, GearDisplay2, GearFontSize, GearIcon, GearLook, GearSize, GearText, GearXY, GGraph, GGroup, GImage, GLabel, GList, GLoader, GLoader3D, GMovieClip, GObject, GObjectPool, GProgressBar, GRichTextField, GRoot, GroupLayoutType, GScrollBar, GSlider, GTextField, GTextInput, GTree, GTreeNode, GTween, GTweener, Image, ListLayoutType, ListSelectionMode, LoaderFillType, MovieClip, ObjectPropID, ObjectType, OverflowType, PackageItem, PackageItemType, PopupDirection, PopupMenu, ProgressTitleType, registerFont, RelationType, ScrollBarDisplayType, ScrollPane, ScrollType, Transition, TranslationHelper, UBBParser, UIConfig, UIObjectFactory, UIPackage, VertAlignType, Window };