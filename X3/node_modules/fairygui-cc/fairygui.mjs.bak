import { gfx, UIRenderer, Event as Event$1, Vec2, Node, game, director, macro, Color, Layers, Font, resources, Vec3, Rect, UITransform, UIOpacity, isValid, Component, Graphics, misc, Sprite, Size, screen, view, ImageAsset, AudioClip, BufferAsset, AssetManager, Asset, assetManager, Texture2D, SpriteFrame, BitmapFont, sp, dragonBones, path, EventTarget, Mask, math, Label, LabelOutline, LabelShadow, SpriteAtlas, RichText, sys, EventMouse, View, AudioSourceComponent, EditBox, Overflow } from 'cc';
...
class UIPackageHelper {
    //支持框架内的扩展类，框架不使用fgui提供的扩展方式，框架的扩展类不需要继承fgui中的类
    //扩展类统一有一个view属性，view属性就是fgui中的类实例
    static getView(obj) {
        // if(!(obj instanceof GObject)){
        //     return obj.view;
        // }
        return obj && (obj.view || obj);
    }
    static getExtension(obj) {
        return obj && (obj.extension || obj);
    }
    static callOnInit(g) {
        // 证明g为扩展类，则调用一下onInit
        // @ts-ignore
        // if (g instanceof FguiExtensionView) {
        //     g.onInit();
        // }
    }
    static callOnHide(g) {
        // if (g instanceof GComponent) {
        //     let chs = g._children;
        //     if (chs && chs.length > 0) {
        //         for (let i = 0, len = chs.length; i < len; ++i) {
        //             this.callOnHide(chs[i]);
        //         }
        //     }
        //     g = this.getExtension(g);
        //     // @ts-ignore
        //     if (g instanceof FguiExtensionView) {
        //         if (g._lifeState !== 2) {
        //             g.onCloseView();
        ...
        //     }
        // }// @ts-ignore
        // else if (g instanceof FguiExtensionView) {
        //     if (g.view) {
        //         this.callOnHide(g.view)
        //     }
        // }
    }
    static callOnShow(g) {
        ...
        // else if (g instanceof FguiExtensionView) {
        //     if (g.view) {
        //         this.callOnShow(g.view)
        //     }
        // }
    }
    static callOnDestroy(g) {
        // if (g instanceof GComponent) {
        //     let chs = g._children;
        //     if (chs && chs.length > 0) {
        //         for (let i = 0, len = chs.length; i < len; ++i) {
        //             this.callOnDestroy(chs[i]);
        //         }
        //     }
        //     g = this.getExtension(g);
        //     // @ts-ignore
        ...
        // else if (g instanceof FguiExtensionView) {
        //     if (g.view) {
        //         this.callOnDestroy(g.view)
        //     }
        // }
    }
    static createArtAtlas(atlasUrl, imgUrl, atlasName) {
        // let atlasData = Laya.Loader.getRes(atlasUrl);
        // if (!atlasData) {
        //     return;
        // }
        // Laya.Loader.clearRes(atlasUrl);
        // let byte = new Laya.Byte(atlasData);
        // let textureAtlas = Laya.loader.getRes(imgUrl);
        // let count = byte.readUint16();
        // for (let i = 0; i < count; ++i) {
        //     let spriteUrl = "res/art/" + atlasName + "/" + byte.readString();
        //     let texture = Laya.Texture.create(textureAtlas,
        //         byte.readUint32(), byte.readUint32(),
        //         byte.readUint32(), byte.readUint32(),
        //         byte.readUint32(), byte.readUint32(),
        //         byte.readUint32(), byte.readUint32());
        //     Laya.Loader.cacheTexture(spriteUrl, texture);
        // }
    }
    static callSetData(g, data) {
        // if (g instanceof GComponent) {
        //     g = this.getExtension(g);
        // }
        // //@ts-ignore
...
class ComponentParser extends BaseParser {
    ...
    _parseNextChild2(componentObj, buffer, componentPI, completeHandler) {
        ...
        child = UIPackageHelper.getView(child);
        if (pi) {
            if (completeHandler) {
                //@ts-ignore
    ...
    _parseNextChild3(buffer, componentObj, child, bindName, completeHandler = null) {
        ...
        if (completeHandler) {
            ...
            parser.completeHandler = () => {
                ...
                if (bindName) {
                    //@ts-ignore
                    componentObj[child._name] = UIPackageHelper.getExtension(child);
                }
                completeHandler();
            };
    ...
    /*_parseCallChildrenInitFun(componentObj: GComponent, buffer: ByteBuffer, componentPI: PackageItem) {
        let childCount = componentObj._children.length;
        for (let i = 0; i < childCount; ++i) {
            let ex = UIPackageHelper.getExtension(componentObj._children[i]);
            //@ts-ignore
            // if (ex instanceof FguiExtensionView) {
            //     //@ts-ignore
            //     ex.onInit();
            // }
        }
...
class UIPackage {
    ...
    nextCreateAsync() {
        ...
        parser.parse(UIPackageHelper.getView(view), pi, true, () => {
            UIPackageHelper.recoverParser(parser);
    ...
    internalCreateObject(item, userClass) {
        ...
        parser.parse(UIPackageHelper.getView(g), item);
        UIPackageHelper.recoverParser(parser);
        constructingDepth.n--;
        return g;
    }
...
class GTextField extends GObject {
    ...
    get stroke() {
        return this._label.enableOutline ?  this._label.outlineWidth : 0; //(this._outline && this._outline.enabled) ? this._outline.width : 0;
    }
    set stroke(value) {
        if (value == 0) {
            /*
...
class GList extends GComponent {
    ...
    dispose() {
        this.clearAnimation();
        this._partner.unschedule(this._frameShowItem);
    ...
    get selectedIndex() {
        var i;
        if (this._virtual) {
            for (i = 0; i < this._needShowNums /**this._realNumItems**/; i++) {
    ...
    set numItems(value) {
        if (this._virtual) {
            ...
            let lastShowNums = this._realNumItems || 0; //上次已渲染数量
            ...
            //逐帧显示
            let count = this._needShowNums = this._realNumItems;
            this._realNumItems = Math.min(lastShowNums, count); //修正已渲染数量
            this._partner.unschedule(this._frameShowItem);
            if (count === this._realNumItems) {
                this._refreshVirtualList();
                this.scrollPane.touchEffect = true;
                this.node.emit(Event.LIST_SHOW_END);
            }
            else {
                this.scrollPane.touchEffect = false;
                let times = count - this._realNumItems;
                this._partner.schedule(this._frameShowItem, 0, times);
            }
        }
        else {
            var cnt = this._children.length;
    ...
    _frameShowItem() {
        ...
        if (_t._needShowNums - _t._realNumItems > 0) {
            _t._realNumItems++;
        }
        else {
            _t.scrollPane.touchEffect = true;
            this.node.emit(Event.LIST_SHOW_END);
        }
    }
    refreshVirtualList() {
    ...
    //添加两个函数定义，游戏内拓展
    playAnimation(type, data = null, compHandler = null) {
    }
    /**
     * 清理动画
     * @param force 强制清理
     */
    clearAnimation(force = true) {
    }
}
var s_n = 0;
